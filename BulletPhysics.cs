...
public class BulletPhysics : MonoBehaviour
{
    ...
    private float MuzzleVelocity = 900;
    ...
    void Start()
    {   
        var gObj = GameObject.Find("muzzleRef");
        ...
        m_bullet = new Bullet(150f, 822f, 10f, 0.0078232f, 0.0338328f, 101325f, 288.16f ,barrel_Pos);
        m_bullet.fireBullet();
        bulletInflight = true;
    ...
    {   
        ...
        if (Input.GetMouseButtonDown(0)){
            ...
            if (bulletInflight == false) {
                m_bullet = new Bullet(150f, 50f, 10f, 0.0078232f, 0.0338328f, 101325f, 288.16f ,m_Rigidbody.position);
                m_bullet.fireBullet(175);
                bulletInflight = true;
            }
        }

        if (m_bullet.projectile_Despawn == false && bulletInflight ==true) {
    ...
    void OnTriggerEnter(Collider other) {
        ...
        m_bullet.projectile_Despawn = true;
    }
...
public class Bullet
{
    ...
    public Bullet( float grains, float muzzleVelocity, float barrelTwist, float bulletDia, float bulletLen, float pressure, float temp, Vector3 initialPos)
    {
    ...
    public void fireBullet()
    {
        ...
        muzzleRef = GameObject.Find("muzzleRef");
        Vector3 StartRotation = muzzleRef.transform.eulerAngles ;

        this.velocity_Vector[0] =  this.muzzle_Velocity * Mathf.Cos(( (StartRotation.y -180 ) * Mathf.PI / 180 ));
        this.velocity_Vector[1] = -this.muzzle_Velocity * Mathf.Sin(( (StartRotation.y -180 ) * Mathf.PI / 180 )) * Mathf.Sin(( (StartRotation.z  ) * Mathf.PI / 180 ));
        this.velocity_Vector[2] =  this.muzzle_Velocity * Mathf.Sin(( (StartRotation.y -180 ) * Mathf.PI / 180 )) * Mathf.Cos(( (StartRotation.z  ) * Mathf.PI / 180 ));
